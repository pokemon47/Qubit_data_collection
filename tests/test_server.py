...
class testQubitApis(unittest.TestCase):
    ...
    def test_set_up(self):
        ...
        self.assertEqual(response.data.decode("utf-8"), "Hello, Flask is working!")
    ...
    def test_newsapi_missing_name(self):
        ...
        response = self.app.get("/newsapi?from_date=2025-03-04&to_date=2025-03-10")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.get_json(), {"error": "Invalid 'name' given"})

    def test_newsapi_invalid_name(self):
        # name has AND operation --> invalid name --> return code 400 + err msg
        response = self.app.get("/newsapi?name=Facebook+AND+Apple&from_date=2025-03-04&to_date=2025-03-10")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.get_json(), {"error": "Invalid 'name' given"})
    ...
    def test_newsapi_missing_dates(self):
        ...
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.get_json(), {"error": "Please provide both to and from dates or none"})

    def test_newsapi_invalid_date_format(self):
        # incorrect date format -> 400 + err msg
        response = self.app.get("/newsapi?name=Facebook&from_date=03-04-2025&to_date=2025/03/10")
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.get_json(), {"error": "Date values must be of ISO 8601 format (e.g. 2025-03-04 or 2025-03-04T07:11:59)"})
